...
def create_anomaly(entry: schemas.AnomalyDetectionCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao criar anomaly: {e}")
...
def update_anomaly(entry_id: int, updated: schemas.AnomalyDetectionUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar anomaly: {e}")
...
def delete_anomaly(entry_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao deletar anomaly: {e}")
