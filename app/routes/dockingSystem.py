...
def create_docking(entry: schemas.DockingSystemCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error creating docking entry: {e}")
...
def update_docking(entry_id: int, updated: schemas.DockingSystemUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error updating docking entry: {e}")
...
def delete_docking(entry_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error deleting docking entry: {e}")
