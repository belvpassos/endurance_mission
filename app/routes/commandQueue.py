...
def create_command(entry: schemas.CommandQueueCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao criar command: {e}")
...
def update_command(entry_id: int, updated: schemas.CommandQueueUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar command: {e}")
...
def delete_command(entry_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao deletar command: {e}")
